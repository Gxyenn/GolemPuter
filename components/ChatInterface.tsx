...
import { getPuterResponse } from '../services/puter'; // Ganti service ke Puter
...
const Typewriter = ({ text, onComplete, speed = 1 }: { text: string; onComplete?: () => void; speed?: number }) => {
  ...
  useEffect(() => {
    // Speed optimization: chunking characters for long responses
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const handleSend = async () => {
    if (!input.trim() || !activeChatId || !activeChat || isTyping) return;
    ...
    // Call Puter Service
    const aiResponseContent = await getPuterResponse(currentInput, activeChat.messages);
          ...
          <div className="flex-1 flex justify-center min-w-0">
            <div className="text-center group cursor-default max-w-full">
              ...
              <div className="flex items-center justify-center gap-1.5">
                ...
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Puter Systems Active</span>
              </div>
            </div>
          </div>
          ...
          {activeChat?.messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-1000">
              ...
              <p className="text-zinc-400 max-w-sm leading-relaxed font-medium">
                ...
                Running on Kimi Engine via Puter.js. Ready to code!
              </p>
              <div className="mt-10 grid grid-cols-2 gap-3 max-w-md w-full">
                <div className="glass p-4 rounded-2xl text-xs text-zinc-500 font-bold border-blue-500/10 hover:border-blue-500/30 transition-colors">"Analyze this Algorithm"</div>
          ...
          <div className="max-w-4xl mx-auto relative">
            ...
            <div className="relative bg-[#111113]/40 backdrop-blur-3xl rounded-[2.5rem] p-1.5 shadow-[0_10px_50px_-10px_rgba(0,0,0,0.5)] border border-white/5 transition-all duration-500 hover:border-blue-500/20 group/input">
              <div className="flex items-end gap-1 px-2">
                {/* Visual Icon (Since Upload is gone, we put a Cpu/AI icon here purely for aesthetics) */}
                <div className="p-4 text-zinc-600 flex items-center justify-center shrink-0">
                   <Cpu size={20} />
                </div>
